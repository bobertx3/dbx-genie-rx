...
class GenieSpaceOptimizer:
    ...
    def generate_optimizations(
        ...
        """
        logger.info("Starting optimization generation")
        ...
        feedback_dicts = [
            {
                ...
                "user_overrode_auto_label": item.user_override,
                "auto_comparison_summary": item.auto_label_reason,
            }
            for item in labeling_feedback
        ]

        # Discover missing joins
        ...
        logger.info(f"Optimization prompt length: {len(prompt)} chars")

        # Call the LLM with extended token budget for diagnosis output
        ...
        logger.info(f"LLM responded, content length: {len(content)} chars")
